...
public class JsonExplorer {
    ...
    public JsonExplorer(ContainerFactory factory) {
        ContainerDirector director = new ContainerDirector(factory);
    ...
    public static void main(String[] args) throws IOException {
        ...
        Map<String, Supplier<ContainerFactory>> containerFactoriesMap = new HashMap<>();
        containerFactoriesMap.put("treedefault", () -> new JsonTreeFactory(Icon.DefaultIcon::new));
        containerFactoriesMap.put("treepoker", () -> new JsonTreeFactory(Icon.PokerIcon::new));
        containerFactoriesMap.put("rectangledefault", () -> new RectangleFactory(Icon.DefaultIcon::new));
        containerFactoriesMap.put("rectanglepoker", () -> new RectangleFactory(Icon.PokerIcon::new));
        containerFactoriesMap.put("treeconfig", () -> new JsonTreeFactory(Icon.ConfigIcon::new));
        ...
        String key = styleArg + iconFamilyArg;
        if (containerFactoriesMap.get(key) == null) {
            if (!"tree".equals(styleArg) && !"rectangle".equals(styleArg)) {
                // Handle unknown style parameter
                System.err.println("Unknown style: " + key);
                return;
            }
        } else {
            factory = containerFactoriesMap.get(key).get();
        }
